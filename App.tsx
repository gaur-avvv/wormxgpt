...
import { InlineCode, ExecutionTerminal } from './components/CodeBlock';
            ...
            <ReactMarkdown
              ...
              components={{
                code({ node, className, children, ...props }: any) {
                  ...
                  return (
                    <Suspense fallback={<div className="my-3 p-4 bg-[#0a0505] animate-pulse border-2 border-red-900/30 rounded-lg text-red-500 font-mono text-center tracking-widest text-[10px]">_LOADING_COMPILER_MODULE...</div>}>
  ...
  const handleSend = useCallback(async (forcedText?: string) => {
    ...
    try {
      ...
      for await (const chunk of agentEngine.runReActLoop({
        ...
        settings,
        llmService: serviceToUse,
        streamCallback: () => {} // currently unused since React relies on state directly inside the loop
      })) {
